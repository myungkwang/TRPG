...
export function getSfxVolume(settings = loadSettings(), base = 1) {
  return clamp01(settings.masterVolume * settings.sfxVolume * base)
}
...
export function getQualityPreset(settings = loadSettings()) {
  return QUALITY_PRESETS[settings.quality] || QUALITY_PRESETS.high
}

export function getRendererPixelRatio(settings = loadSettings()) {
  const preset = getQualityPreset(settings)
  return Math.min(window.devicePixelRatio || 1, preset.pixelRatio)
}

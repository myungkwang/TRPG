...
SCENES: dict[str, dict] = {
    "clinic_wake": {
        ...
        "text": (
            ...
            "의사: \"재끝 마을은 영정 채굴로 버티는 곳입니다. 그런데 최근 채굴량은 늘었는데, 갱도에서 "
            "돌아오지 못한 사람들의 이름은 공식 보고서에서 지워지고 있습니다.\"\n"
            "의사: \"밤마다 봉우리에서 안개가 내려오고, 광산 종이 울리면 부상자들이 같은 말을 합니다. "
            "갱도 안쪽에서 기계가 아닌 것이 숨을 쉰다고요.\"\n"
            "의사: \"여관의 린은 소문과 장부를 알고 있고, 감독 가일은 실종자 기록을 숨기고 있습니다. "
            "몸은 움직일 수 있습니다. 다만 이 마을의 안개는, 사람을 쉽게 놓아주지 않습니다. "
    ...
    "mine_gate": {
        ...
        "text": (
            ...
            "GM: 그의 품에서 삐져나온 서류에는 공식 명패와 맞지 않는 숫자가 적혀 있습니다. 이름은 지워져 "
            "있지만, 갱도 번호와 작업조, 그리고 자정 직후의 운송 칸 번호가 같은 줄에 반복됩니다.\n"
            "GM: 당신이 한 장을 낚아채자 가일의 얼굴이 굳습니다. 이것이 광부들이 사라진 뒤에도 공식 "
            "보고에 올라가지 않았던 은폐된 명부입니다.\n"
            "가일: \"그건 작업 배치표일 뿐이오. 밤 운송표와 맞춰 볼 생각은 하지 않는 게 좋을 거요.\""
        ),
        "choices": [
    ...
    "night_watch": {
        ...
        "flag": "FLG_CLUE_02",
        "text": (
            "GM: 당신은 광산 입구와 마을 광장을 잇는 낡은 수레길에 몸을 숨깁니다. 자정이 가까워지자 "
            "광산의 교대 종은 울리지 않는데도 감시병 셋이 동시에 초소를 비웁니다.\n"
            "GM: 기록상 빈 짐칸이어야 할 수레들이 천천히 움직입니다. 첫 번째 수레에는 영석 가루가, "
            "두 번째 수레에는 피가 마르지 않은 작업복이, 세 번째 수레에는 안쪽에서 두드리는 듯한 "
            "희미한 진동이 실려 있습니다.\n"
            "GM: 수레들은 정제소가 아니라 봉우리로 이어지는 샛길로 꺾입니다. 그 순간 멈춰 있던 "
            "술식기계들이 한 박자 늦게 고개를 돌리고, 여관 위층 창가에는 린의 그림자가 아주 잠깐 비칩니다.\n"
            "린: \"저 길은 정제소로 가는 길이 아니에요. 누군가 마을의 공포와 산의 분노가 같은 밤에 "
            "움직이도록 맞춰 두었어요.\"\n"
            "GM: 이제 실종은 사고처럼 보이지 않습니다. 광부와 영석, 그리고 봉우리 표식은 같은 야간 "
            "운송로 위에서 함께 사라지고 있었습니다."
        ),
        "choices": [
    ...
    "deep_mine": {
        ...
        "flag": "FLG_CLUE_04",
        "text": (
            "GM: 갱도 심부로 내려가자 린이 먼저 등불을 낮춥니다. 평소처럼 웃지 않는 얼굴입니다.\n"
            "린: \"여기부터는 제가 숨겨 온 길이에요. 장부의 숫자, 봉우리 표식, 토비의 노래가 모두 "
            "이 아래로 이어져요.\"\n"
            "GM: 새 광맥이라 불리던 것은 광물이 아니라 거대한 둥지의 상처처럼 맥동합니다. 벽을 타고 "
            "흐르는 영정은 광석의 빛이라기보다 오래된 기억이 새어 나오는 피처럼 보입니다.\n"
            "GM: 암벽 너머에서 갇혀 있던 광부들의 두드림이 희미하게 돌아옵니다. 토비는 숨을 삼키고 "
            "그 박자를 따라 손가락을 움직입니다.\n"
            "토비: \"형이 살아 있을지도 몰라요. 저 소리, 광부들이 쓰는 신호예요!\"\n"
            "GM: 토비가 자장가의 첫 소절을 떨리는 목소리로 부르자, 암벽 너머의 두드림이 같은 박자로 "
            "대답합니다. 그 노래는 아이를 재우는 노래가 아니라 봉인을 깨우지 않기 위한 의식의 문장이었습니다.\n"
            "린: \"당신은 이 노래를 배운 적이 없죠. 그런데도 다음 구절을 알고 있어요. 오래전 이 봉인을 "
            "잠재운 사람이 바로 당신이었으니까요.\"\n"
            "GM: 당신의 심장 박동과 벽의 영정맥이 같은 리듬으로 맞물립니다. 카르가스의 둥지, 영정의 "
            "근원, 토비의 자장가, 린이 숨겨 온 진실이 하나의 길로 이어집니다."
        ),
        "choices": [
            ...
            {"id": "deep_follow_lullaby", "text": "자장가의 마지막 구절을 따라 봉우리로 올라간다", "next": "peak_confront", "hidden_flags": ["FLG_MEMORY_RECOVERED", "FLG_LIN_ALLY", "STORY_TOBI_LULLABY"]},
    ...
    "hidden_epilogue": {
        ...
        "ending": True,
        "text": (
            "GM: 봉우리의 바람 앞에서 린은 더 이상 웃지 않습니다. 그녀는 당신을 직함이 아니라 오래전 "
            "이 산에 묻힌 이름으로 부릅니다. 그 이름을 듣는 순간, 토비의 자장가 마지막 구절이 당신의 "
            "입에서 먼저 흘러나옵니다.\n"
            "린: \"기억을 너무 일찍 돌려주면 봉인이 먼저 열릴까 봐 숨겼어요. 하지만 이제는 숨기는 쪽이 "
            "더 큰 거짓이 됐어요. 당신은 이 마을에 우연히 온 사람이 아니에요.\"\n"
            "GM: 자장가의 박자에 맞춰 영정맥이 빛납니다. 광부들이 암벽 너머에서 보낸 두드림, 장부에 찍힌 "
            "봉우리 표식, 마르타가 지킨 의례의 마지막 구절이 모두 같은 문장이었음을 깨닫습니다.\n"
            "카르가스: \"작은 인간아. 너는 돌아온 것이로구나. 내 둥지를 잠재우고, 너 자신의 이름까지 "
            "봉인했던 약속의 증인아.\"\n"
            "GM: 영정은 단순한 광물이 아니었습니다. 오래전 카르가스의 상처와 인간의 맹세가 뒤엉기며 새어 "
            "나온 기억의 잔재였습니다. 제국은 그것을 연료로 팔았고, 린은 전쟁을 막겠다는 명분으로 그 "
            "흐름을 조종했으며, 마을은 진실을 잊은 채 두려움만 물려받았습니다.\n"
            "린: \"나는 진실을 이용했어요. 당신은 진실을 감당하려 했고요. 이번에는 제가 도망치지 않을게요.\"\n"
            "GM: 마르타는 오래된 의례를 끝까지 잇고, 토비는 암벽 너머에서 돌아온 형의 대답을 듣습니다. "
            "카르가스의 분노는 사라지지 않지만, 더 이상 방향 없는 저주로 마을을 덮치지 않습니다.\n"
            "GM: 재끝 마을의 이야기는 처음부터 당신의 이야기이기도 했습니다. 안개는 완전히 사라지지 "
            "않지만, 이제 그 안개는 잃어버린 이름을 가리는 장막이 아니라 돌아올 길을 품는 새벽의 숨결로 남습니다."
        ),
        "choices": [],
    },
}
